...
class Ball:
    def __init__(
        self, space: pm.Space, *, pos, mass, color, radius=None, btype=pm.Body.DYNAMIC
    ) -> None:
...
class Segment:
    ...
    def draw(self, screen: pg.surface) -> None:
        self.pg = pg.draw.polygon(
            screen, color=self.shape.color, points=self.calculate_polygon_points()
        )
...
class Simulation:
    def __init__(self) -> None:
        ...
        self.funnel = [
            Segment(
                self.space, pos_a=(0, 0), pos_b=(0, 120), color=BOX_COLOR, thickness=5
            ),
            Segment(
                self.space,
                pos_a=(0, 120),
                pos_b=(WIDTH / 2 - 30, 200),
                color=BOX_COLOR,
                thickness=5,
            ),
            Segment(
                self.space,
                pos_a=(WIDTH / 2 - 30, 200),
                pos_b=(WIDTH / 2 - 30, 260),
                color=BOX_COLOR,
                thickness=2.5,
            ),
            Segment(
    ...
    def step(self):
        for event in pg.event.get():
            ...
            if event.type == pg.MOUSEBUTTONDOWN:
                ...
                self.balls.extend(
                    [
                        Ball(self.space, pos=pos, color=(200, 170, 0), mass=20)
                        for _ in range(50)
                    ]
                )
